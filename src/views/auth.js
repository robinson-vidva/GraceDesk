...
const csrfInput = (token) => html`<input type="hidden" name="_csrf" value="${token}" />`;
...
export function loginPage(ctx, env, { error, email = '' } = {}) {
  const inner = html`
    ...
    <div class="mt-4 text-sm text-center space-y-1">
      <div><a href="/forgot-password" class="text-brand hover:underline">Forgot your password?</a></div>
      <div class="text-slate-500">New here? <a href="/register" class="text-brand hover:underline">Register</a></div>
    </div>`;
  return layout({ ...ctx, title: 'Login' }, authCard('Log in', inner));
}
...
export function registeredPage(ctx) {
  const inner = html`
    ...
    <div class="mt-4"><a href="/" class="text-brand hover:underline text-sm">← Back to home</a></div>`;
...
export function forgotPage(ctx, env, { error, sent, devLink } = {}) {
  if (sent) {
    const inner = html`
      ...
      <div class="mt-4"><a href="/login" class="text-brand hover:underline text-sm">← Back to login</a></div>`;
  ...
  const inner = html`
    ...
    <div class="mt-4 text-sm text-center"><a href="/login" class="text-brand hover:underline">Back to login</a></div>`;
...
export function resetPage(ctx, { token, error, invalid } = {}) {
  if (invalid) {
    const inner = html`
      ...
      <div class="mt-2"><a href="/forgot-password" class="text-brand hover:underline text-sm">Request a new link</a></div>`;
...
export function changePasswordPage(ctx, { error, forced } = {}) {
  ...
  return layout({ ...ctx, title: 'Change password' },
    authCard('Change your password', inner));
}
